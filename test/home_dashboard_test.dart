...
void main() {
  for (final width in [390.0, 1440.0]) {
    for (final dark in [false, true]) {
      testWidgets('dashboard fits ${width.toInt()}px in ${dark ? 'dark' : 'light'} theme', (tester) async {
        ...
        final items = [
          LaterBoxItem(id: 'due', title: 'Finish portfolio', type: 'task', text: 'Finish portfolio',
            status: ItemStatus.deferred, createdAt: now, returnAt: now.toUtc()),
          LaterBoxItem(id: 'future', title: 'Read design trends', type: 'link', url: 'https://example.com',
            status: ItemStatus.deferred, createdAt: now, returnAt: now.add(const Duration(days: 1)).toUtc()),
          LaterBoxItem(id: 'someday', title: 'An idea for later', type: 'note', text: 'An idea for later',
            status: ItemStatus.deferred, createdAt: now),
        ];
        final boundary = GlobalKey();
        await tester.pumpWidget(ProviderScope(overrides: [
          guestModeProvider.overrideWith((ref) => true), appDatabaseProvider.overrideWithValue(db),
          allItemsProvider.overrideWith((ref) => Stream.value(items)),
          scheduleNowProvider.overrideWithValue(() => now),
        ], child: MaterialApp(theme: dark ? ThemeData.dark(useMaterial3: true) : AppTheme.light,
          home: RepaintBoundary(key: boundary, child: const HomeShell(selectedIndex: 0, child: HomeDashboard())))));
        ...
        if (screenshots.isNotEmpty) {
          ...
          await tester.runAsync(() => File('$screenshots/dashboard-${width.toInt()}-${dark ? 'dark' : 'light'}.png').writeAsBytes(png!.buffer.asUint8List()));
          image!.dispose();
        }
  ...
  testWidgets('capture fits a small phone with its keyboard open', (tester) async {
    ...
    await tester.pumpWidget(MaterialApp(home: MediaQuery(data: const MediaQueryData(
      size: Size(360, 640), viewInsets: EdgeInsets.only(bottom: 280)), child: const Scaffold(body: CaptureSheet()))));
    await tester.pumpAndSettle();
    expect(tester.takeException(), isNull);
  });
}
