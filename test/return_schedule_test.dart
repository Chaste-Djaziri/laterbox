...
void main() {
  ...
  test('presets resolve in local time and are stored as UTC', () {
    ...
    expect(
      resolveReturnPreset(ReturnPreset.laterToday, now),
      now.add(const Duration(hours: 3)).toUtc(),
    );
    expect(
      resolveReturnPreset(ReturnPreset.tomorrow, now),
      DateTime(2026, 9, 17, 9).toUtc(),
    );
    expect(
      resolveReturnPreset(ReturnPreset.weekend, now),
      DateTime(2026, 9, 19, 9).toUtc(),
    );
    expect(
      resolveReturnPreset(ReturnPreset.weekend, DateTime(2026, 9, 19, 8)),
      DateTime(2026, 9, 26, 9).toUtc(),
    );
    expect(
      resolveReturnPreset(ReturnPreset.weekend, DateTime(2026, 9, 20)),
      DateTime(2026, 9, 26, 9).toUtc(),
    );
  ...
  test('Today includes timed items only on the current local date', () {
    final list = [
      item('today', now),
      item('tomorrow', DateTime(2026, 9, 17, 9)),
      item('someday', null),
      item('archived', now, status: ItemStatus.archived),
    ];
    expect(scheduleItems(list, ScheduleView.today, now).map((i) => i.id), [
      'today',
    ]);
    expect(
      scheduleItems(
    ...
    expect(scheduleItems(list, ScheduleView.upcoming, now).map((i) => i.id), [
      'tomorrow',
    ]);
    expect(scheduleItems(list, ScheduleView.someday, now).map((i) => i.id), [
      'someday',
    ]);
  });

  test(
    'capture, duplicate, reschedule, complete and remote round trip',
    () async {
      ...
      expect(saved.type, 'task');
      expect(saved.returnAt, now.toUtc());
      expect(saved.isDue(now), isTrue);
      expect(
        await repo.save(
          'Do the thing',
          returnAt: now.add(const Duration(days: 1)),
        ),
        'task',
      );
      ...
      final remote = RemoteItem.fromJson({
        'id': 'remote',
        'user_id': 'user-b',
        'type': 'link',
        'favorite': false,
        'status': 'deferred',
        'return_at': now.toUtc().toIso8601String(),
  ...
  test('v13 migration keeps old Inbox visible and saved/archived intact on restart', () async {
    ...
    expect((await reopened.itemById('inbox'))!.returnAt, now);
    await reopened.close();
  });

  testWidgets(
    'clock wakes Inbox at deadline and on resume, rescheduling hides it',
    (tester) async {
      final db = AppDatabase(NativeDatabase.memory());
      ...
      await db.saveItem(
        ItemsCompanion.insert(
          ...
          returnAt: Value(current.add(const Duration(minutes: 5))),
          createdAt: current,
          updatedAt: current,
        ),
      );
          ...
          child: MaterialApp(
            home: Consumer(
              builder: (_, ref, _) => Text(
                '${ref.watch(inboxItemsProvider).valueOrNull?.length ?? 0} due',
              ),
            ),
          ),
        ),
      ...
      await tester.runAsync(
        () => container
            ...
            .reschedule('timed', current.add(const Duration(hours: 1))),
      );
      await tester.pumpAndSettle();
      expect(find.text('0 due'), findsOneWidget);
  ...
  testWidgets('picker defaults to Someday and displays resolved time', (
    tester,
  ) async {
    DateTime? value;
    await tester.pumpWidget(
      MaterialApp(
        home: Scaffold(
          body: StatefulBuilder(
            builder: (context, setState) => ReturnTimePicker(
              value: value,
