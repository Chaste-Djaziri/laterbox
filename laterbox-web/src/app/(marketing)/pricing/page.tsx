    ...
    <main className="min-h-screen bg-[#f7f5ee] px-5 py-14 text-[#171711] sm:py-20">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mt-10 grid gap-5 lg:grid-cols-2">
          ...
          <PlanCard featured title="LaterBox Pro" badge="14-day trial for eligible subscribers" price={localizedPrice} suffix={interval === 'month' ? '/month' : '/year'} description="Sync everywhere and capture useful content before it slips away." features={proFeatures} action={isPro ? <button type="button" onClick={() => void run('manage')} disabled={busy !== null || entitlement.provider !== 'paddle'} className="w-full rounded-2xl bg-[#d7ff27] px-5 py-3 text-sm font-black text-black disabled:opacity-50">{busy === 'manage' && <Loader2 className="mr-2 inline size-4 animate-spin" />}{presentation.actionLabel}</button> : user ? <button type="button" onClick={() => void run(interval)} disabled={busy !== null} className="w-full rounded-2xl bg-[#d7ff27] px-5 py-3 text-sm font-black text-black disabled:opacity-50">{busy === interval && <Loader2 className="mr-2 inline size-4 animate-spin" />}Start free trial</button> : <Link href={`/login?next=${encodeURIComponent(`/pricing?plan=${interval}`)}`} className="block rounded-2xl bg-[#d7ff27] px-5 py-3 text-center text-sm font-black text-black">Sign in to start trial</Link>} />
        </div>
